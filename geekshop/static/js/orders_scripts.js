...
//    $('.order_form select').change(function (event) {
    $('.order_form').on('change', 'select', function (event) {
        ...
        var orderitem_product_pk = event.target.options[event.target.selectedIndex].value;

        if (orderitem_product_pk) {
            //console.log('orderitem num', orderitem_num, ' | product pk:', orderitem_product_pk);
            $.ajax({
                ...
                success: function (data) {
                    if (data.price) {
                        ...
                        var $productQuantity = $current_tr.find('input[type="number"]');
                        console.log(isNaN($productQuantity.val()));
                        $productQuantity.val(0);
                        if (isNaN($productQuantity.val())) {
                            $productQuantity.val(0);
                        }
                        orderSummaryRecalc();
                    }
